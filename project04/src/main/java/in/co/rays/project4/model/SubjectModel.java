...
//        SubjectBean duplicateName = findByName(bean.getCourse_Id());
        int pk = 0;
...
// bean.setCourseName(CourseBean.getName());
 ...
 try {
     ...
     conn.setAutoCommit(false); // Begin transaction
     PreparedStatement pstmt = conn
             .prepareStatement("UPDATE ST_SUBJECT SET Subject_Name=?,Course_NAME=?,Course_ID=?,Discription=?,CREATEDBY=?,MODIFIEDBY=?,CREATEDDATETIME=?,MODIFIEDDATETIME=? WHERE ID=?");
