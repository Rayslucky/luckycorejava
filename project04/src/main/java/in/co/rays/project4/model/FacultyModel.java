...
//		 
	...
	public void update(FacultyBean bean) throws ApplicationException, DuplicateRecordException {
		...
		PreparedStatement pstmt = conn.prepareStatement(
				"UPDATE ST_FACULTY SET  FIRST_NAME=?, LAST_NAME=?, GENDER=?, DOB=?,QUALIFICATION=?, EMAIL_ID=?, MOBILE_NO=? , COLLEGE_ID=?, COLLEGE_NAME=?,COURSE_ID=?,COURSE_NAME=?, SUBJECT_ID=?, SUBJECT_NAME=?, CREATEDBY=? , MODIFIEDBY=? , CREATEDDATETIME=? , MODIFIEDDATETIME=?  WHERE ID= ? ");
			...
			pstmt.setString(11, bean.getCollege_Name());
